...
int main() {
  ...
  std::cout << "Gauss-Seidelov cas (serijski): " << time_duration.count() << " sekunde" << std::endl;
  ...
  std::cout << "Maksimalna temperatura (serijska): " << max_T << " °C." << std::endl;

  // Preverjanje moznosti paralelizacije.
  // Preverimo, ali ima matrika A vse nenicelne diagonalne elemente.
  ...
  // Paralelizacija ni mozna, ce so diagonalni elementi nic.
  if (!can_parallelize) 
  {
      std::cout << "Paralelizacija ni mozna, ker matrika A vsebuje nicelne diagonalne elemente." << std::endl;
      return 0;
  }

  // Paralelizacija z uporabo OpenMP
...
#pragma omp parallel for // OpenMP za paralelno izvajanje.
      for (int i = 0; i < n; i++) 
      {
          double sigma = 0.0; // Del vsote, ki vkljucuje vse elemente razen diagonalne.
          for (int j = 0; j < n; j++) 
          {
              if (j != i) // Izkljucimo prispevek diagonalnega elementa.
              { 
  ...
  std::cout << "Gauss-Seidelov cas (paralelni): " << time_duration.count() << " sekunde" << std::endl;
  ...
  std::cout << "Maksimalna temperatura (paralelna): " << max_T << " °C." << std::endl;
